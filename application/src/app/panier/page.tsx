          ...
          {Object.entries(panierParEvenement).length === 0 ? (
            ...
            <>
                ...
                return (
                  <div key={evenementId} className="mb-8 border-b pb-4">
                    <h2 className="text-lg md:text-xl font-semibold mb-2 text-accent flex flex-wrap items-center gap-2">
                      ...
                      {evenement && (
                        <span className="ml-2 text-sm md:text-base font-normal text-black bg-base-200 rounded px-2 py-1">Places restantes : {remainingTickets}</span>
                      )}
                    </h2>
                    <ul className="divide-y divide-base-200">
                      {items.map(item => {
                        ...
                        return (
                          <li key={item.offreId} className="flex flex-wrap md:flex-nowrap items-center justify-between py-2 text-black gap-2 md:gap-4">
                            <span className="font-medium text-black min-w-[100px] md:min-w-[140px] truncate">{offre ? offre.libelle : `Offre #${item.offreId}`}</span>
                            <div className="flex items-center gap-1 md:gap-2">
                              <button
                                className="p-1 rounded hover:bg-gray-200 disabled:opacity-50 border border-base-200"
                                onClick={() => dispatch(removeOneArticleFromCart({ evenementId: item.evenementId, offreId: item.offreId }))}
                              ...
                              <button
                                className="p-1 rounded hover:bg-gray-200 disabled:opacity-50 border border-base-200"
                                disabled={!canAdd}
                                onClick={() => dispatch(addOneArticleToCart({ evenementId: item.evenementId, offreId: item.offreId }))}
                            ...
                            <button
                              ...
                              onClick={() => {
                                for (let i = 0; i < item.quantity; i++) {
                                  dispatch(removeOneArticleFromCart({ evenementId: item.evenementId, offreId: item.offreId }));
                                }
                              }}
              ...
              <div className="flex flex-col md:flex-row items-center justify-between mt-8 gap-4">
                <div className="text-xl font-bold text-black">Total : <span className="text-accent">{totalPanier} €</span></div>
                <button
